...
st.markdown("""
### 📖 활동 소개
소비전력($W$)과 사용 시간($h$)이 만나 전기에너지($kWh$)가 결정되는 원리를 배운 고심이!
이제 **과학적 시뮬레이터**를 통해 나의 생활 데이터를 입력하고, 
미션을 조작하여 **돈을 들이지 않고도 1등급 가전 교체 이상의 탄소를 줄일 수 있는 나만의 탄소 다이어트 계획**을 세워봅시다.
""")
...
st.caption("고심이네 집 기본 데이터를 바탕으로, 나의 평소 생활 습관 수치를 설정해 보세요.")
...
# -----------------------------------------------------------------------------
st.markdown("### 🎯 Step 2: 나만의 탄소 다이어트 시뮬레이션 (After)")
...
# -----------------------------------------------------------------------------
# 7. 나의 데이터 기반 생태전환 행동 선언문 (Step 4: 포트폴리오 저장)
# -----------------------------------------------------------------------------
st.markdown("### 📝 Step 4: 나의 데이터 기반 생태전환 행동 선언문")
