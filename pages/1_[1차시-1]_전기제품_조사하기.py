...
appliance_options = ["TV", "세탁기", "선풍기", "전기 밥솥", "전기 주전자", "전등", "청소기"]
...
if not (
    ...
    st.info("📌 위 활동의 입력과 분석을 모두 마치면 아래 활동 2가 열립니다.")
    st.stop()
...
# -------------------------------------------------------------------------
st.markdown("---")
st.markdown("### 📊 [사전 탐구] 실제 전기 주전자 데이터 둘러보기")
st.caption("실제 데이터(`data/kettles.csv`)에서 불러온 용량(L)과 소비전력(W) 데이터입니다.")
...
if is_csv_loaded and df_30 is not None:
    ...
    with tab_hist:
        fig_hist = px.histogram(
            ...
            x="소비전력(W)",
            nbins=10,
            title="<b>[수집 데이터] 전기 주전자 소비전력(W) 분포</b>",
    ...
    with tab_scatter:
        fig_scatter = px.scatter(
            ...
            title="<b>[관계 분석] 용량(L)에 따른 소비전력(W) 산점도</b>",
    ...
    * 용량이 클수록 물을 한번에 많이 끓여야 해서 높은 소비전력이 필요할 수 있습니다.
    * 하지만 **같은 용량(예: 1.5L)이라도 빠르게 끓이는 고속형(1800W)과 천천히 끓는 절전형(900W)**이 공존합니다!
    """)
else:
    st.warning(
...
st.success("""
🎯 **[고심이의 1차 필터링 결론]**  
고심이는 시중의 다양한 제품 중 **자신의 방 크기와 마음에 드는 디자인/용량**을 고려하여 **최종 후보 3개(A, B, C)**로 선택지를 줄였습니다!  
이제 3개 제품 중 **성능(속도)과 소비전력(W)**을 비교하여 최고의 전기 주전자를 결정해 봅시다.
""")
